 ...
 * the device (see SymmetricKeyProviderItf). The key is never exchanged over the network. The IV is
 * not derived or exchanged separately either: the encrypted file is the AES-256-GCM IV (12 bytes), the
 * ciphertext and the 16-byte authentication tag, so producing an encrypted blob needs no coordination
 * beyond generating a random IV and prepending it to the AES-GCM output.
 */
